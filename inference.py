...
def test(args, model, test_loader, device):
    ...
    for test_batch in test_loader:
        keep_node_index = []
        keep_node_score = []
        query_id, doc_id = test_batch['query_id'], test_batch['doc_id']
        with torch.no_grad():
            if args.model == 'bert':
                batch_score, _ = model(test_batch['input_ids'].to(device), test_batch['input_mask'].to(device), test_batch['segment_ids'].to(device))
            elif args.model == 'graph':
                batch_score, keep_node_index, keep_node_score = model(test_batch['input_ids'].to(device),
                                                                      test_batch['input_mask'].to(device),
                                                                      test_batch['segment_ids'].to(device),
                                                                      test_batch['token_starts'].to(device),
                                                                      test_batch['q_term_nodes'].to(device),
                                                                      test_batch['doc_term_nodes'].to(device),
                                                                      test_batch['section'].to(device),
                                                                      test_batch['sen_len'].to(device),
                                                                      test_batch['doc_sen_idxes'].to(device),
                                                                      test_batch['rgcn_adjacency'].to(device))
            ...
            if keep_node_index:
                for (q_id, d_id, n_idx, n_score) in zip(query_id, doc_id, keep_node_index, keep_node_score):
                    if q_id not in node_index_dict:
                        node_index_dict[q_id] = {}
                        node_score_dict[q_id] = {}
                    if d_id not in node_index_dict[q_id]:
                        node_index_dict[q_id][d_id] = n_idx.cpu().tolist()
                        node_score_dict[q_id][d_id] = n_score.cpu().tolist()

    return rst_dict, node_index_dict, node_score_dict
...
def main():
    ...
    parser.add_argument('-node_index_path', type=str, default='./results/node_index.tsv')
    args = parser.parse_args()
    print('testing ', args.model)
    om.set_environment(2021, True)
    ...
    rst_dict, batch_node_index, batch_node_score = test(args, model, test_loader, device)
    om.utils.save_trec(args.res, rst_dict)
    if batch_node_index:
        om.utils.save_index(args.node_index_path, batch_node_index, batch_node_score)
